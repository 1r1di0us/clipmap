...
{	
	...
	if(init) // begin initialization
	{
		...
		loopj(0,height) loopi(0,width) // for loops, 0 < j < height and height and 0 < i < width
		{
			float x = float(i) / float(width); // x = i / width (gradually increasing % of width)
			float y = float(j) / float(height); // y = j / height (gradually increasing % of height)
		...
		// terrain texture
		loopj(0, height) loopi(0, width) loopk(0, 3) // // for loop, 0 < j < height, 0 < i < width, 0 < k < 3
		{
